        ...
        if(isset($_POST['SEARCH'])){
            ...
            if($data){
                ...
                foreach($data as $rows){
                    ...
                    $SQL2 = 'SELECT `status` FROM `student_attendance` WHERE `student_id`=:student_id AND `branch_id`=:branch_id AND `faculty_id`=:faculty_id AND `course_id`=(SELECT `id` FROM `course` WHERE `course_code`=:course_code limit 1) AND `date`=:currentDate';
                    ...
                    if($data2){
                        $data[$counter]['ATTENDANCE_STATUS'] = $data2['status'];
                    }
                }
