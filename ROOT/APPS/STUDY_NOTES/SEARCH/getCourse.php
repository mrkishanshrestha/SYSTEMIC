...
include_once '../../../../SYSTEM/IMPORT/BACKEND/kali.php';
    ...
    try{
        if(isset($_POST['SELECT_OPTION'])){
            ...
            $sql = 'SELECT `id`, `course_name`, `course_code`, `credit_hrs`, `lecture_hrs`, `tutorial_hrs`, `lab_hrs`, `faculty_id`, `faculty_period`, `course_document` FROM `course` WHERE `faculty_id`=(SELECT id FROM faculty WHERE short_name=:short_name ) AND `faculty_period`=:faculty_period';
            $data = $kali->kaliPulls($sql,['faculty_period'=>$_POST['FACULTY_PERIOD'],'short_name'=>$_POST['FACULTY']]);
            if($data){
                $kali->kaliReply($data);
            }else{
