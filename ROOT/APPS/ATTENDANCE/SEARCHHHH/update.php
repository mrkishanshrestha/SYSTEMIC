...
if(isset($_POST['UPDATE']) && $_POST['UPDATE']=="JACKX_UPDATE"){
    ...
    $USER_POST = $kali->dataCheck(['DATA'=>'USER_POST','CASE'=>'STRING','CHARACTER'=>'UPPER']);
    ...
    $SQL = ' UPDATE `user` SET `id`=:id,`post`=:post,`first_name`=:first_name,`middle_name`=:middle_name,`last_name`=:last_name,`contact_number`=:contact_number,`phone_number`=:phone_number,`address`=:address,`email`=:email,`document_id`=:document_id,`profile_picture_id`=:profile_picture_id,`description`=:description WHERE `id`=:id'; 
    $PUSH_DATA = [
