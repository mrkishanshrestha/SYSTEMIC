...
include_once '../../../../SYSTEM/IMPORT/BACKEND/kali.php';
    ...
    try{
        ...
        if(isset($_POST['SEARCH'])){
            ...
            `branch_id`=:branch_id AND `faculty_id`=(SELECT `id` FROM `faculty` WHERE `short_name`=:faculty_short_name LIMIT 1) AND `faculty_period`=:faculty_period && (`username` LIKE :value || `roll_no` LIKE :value || `email` LIKE :value || `first_name` LIKE :value )';
            $data = $kali->kaliPulls($sql,['domain_cname'=>$domain,'branch_id'=>$_POST['BRANCH_ID'],'faculty_short_name'=>$_POST['FACULTY_SHORT_NAME'],'faculty_period'=>$_POST['FACULTY_PERIOD'],'value'=>'%'.$_POST['SEARCH_BOX'].'%']);
            if($data){
