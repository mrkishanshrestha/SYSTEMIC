...
if(isset($_POST['UPDATE']) && $_POST['UPDATE']=="JACKX_UPDATE"){
    ...
    if(isset($_FILES['PROFILE_PICTURE'])){
        ...
        $data = $kali->kaliPull($sql,['id'=>$MY_ID]);
        if($data){
            $dir = 'D:/CODE/DEPLOY/SYSTEMIC/ROOT/DATA/USER_DATA/'.$data['username'].'/'.$data['profile_picture_id'];
            if(file_exists($dir)){
                unlink($dir);
            }
    
            $PROFILE_PICTURE =  $kali->uploadFIle(['FILE'=>$_FILES['PROFILE_PICTURE'],'DIR'=>'SYSTEM','TYPE'=>'IMAGE','FOLDER'=>$data['username']]);
