...
<h1> Submitted Assignment Search  </h1>
...
<script>
    ...
    $(document).ready(function(){
        ...
        function loadAssignments(KOHOMAH,COURSE_REFID,AUTHORITY) {
            ...
            var jackxData = juju.postJackx('getSubmittedAssignment.php',{COURSE_REFID: COURSE_REFID});
            ...
            jackxData.done(function(data){
                ...
                juju.makeKaliTable({HEAD:['TITLE','DESCRIPTION','DOCUMENTS','UPLOADED DOCS','MARK','OBTAINED MARK','SUBMITED DATE','DUE DATE',uploadTitle],ID:'ASSIGNMENT_TABLE',APPEND:'searchtable'}).then((data)=>{
                    ...
                    jsonData.forEach(element => {
                        ...
                        var SubmittedlinkHtml = "";
                        submittedPdfs = element.submitted_assignment_files_x;
                        submittedPdfs = submittedPdfs.split(',');
                        submittedPdfs.forEach(filename => {
                            submittedPDFDirectory = "https://www.systemic.com/ROOT/DATA/USER_DATA/"+KOHOMAH+"/ASSIGNMENT/"+element.assignment_refid+'/'+filename;
                            SubmittedlinkHtml += `<a href="`+submittedPDFDirectory+`">`+filename+`</a>`;
                        });
                        ...
                        myPdfs.forEach(filename => {
                            pdfDirectory = "https://www.systemic.com/ROOT/DATA/APP_DATA/ASSIGNMENT/"+element.course_refid+'/'+element.assignment_id+'/'+filename;
                        ...
                        juju.addTableContent({
                            DATA:data,
                            DB_TUPLE_ID:element.assignment_record_id,
                            CONTENT_ID:['TITLE','DESCRIPTION','DOCUMENTS','MARK','DUE_DATE','REMAINING_DAYS',uploadTitle],
                            CONTENT:[
                                element.title,element.description,linkHtml,SubmittedlinkHtml,element.mark,element.mark_obtained,element.due_date,element.due_date,uploadBtn
                            ],
                            COUNTER:SNcounter
                        });
                        SNcounter++;
        ...
        loadAssignments('<?php echo $_SESSION['KOHOMAH']; ?>','<?php echo $_GET['COURSE_REFID']; ?>','<?php echo $_SESSION['MY_AUTHORITY']; ?>');
