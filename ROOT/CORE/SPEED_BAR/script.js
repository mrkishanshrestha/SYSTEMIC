...
function createSpeedBar(appendTo,barTitle,percentageData=0){

    var newId = generateString(5);

    var SpeedBarHtml = 
    `<canvas id="speed-bar-canvas-`+newId+`" height="175" width="350" class="m-auto speed-bar-canvas">
                </canvas>
    <p id="speed-bar-text-`+newId+`" class="speed-bar-text">0 %</p><br>
    <h1>`+barTitle+`</h1>`;

    document.getElementById(appendTo).innerHTML = (SpeedBarHtml);

    totalRot = ((percentageData / 100) * 180 * Math.PI) / 180;

    text = document.getElementById("speed-bar-text-"+newId);
    canvas = document.getElementById("speed-bar-canvas-"+newId);
